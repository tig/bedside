...
- CLI / TUI verbs and flags
- Error messages and exit codes
- Guided multi-step flows (especially body/browser acts)
- Discovery UIs (ports, accounts, devices, clusters)
- Docs generators that leave a Day-2 path

Target audience for surface design: **smart, high-judgment non-experts** and the **agents** driving the tools on their behalf.
...
|-----------------|--------|
| `doctor` | Check host/tool/device readiness; report in plain language; exit non-zero with recovery hints |
| `first-run` / `flash-first` | Own first-time setup from zero once; detect blank vs ready |
| `deploy --verify` | Deploy then prove identity/version; fail closed on mismatch |
| `gate` | Named host proof of done (tests/sim); green means claimable |
...
### 2. Step machines (human body / account)
...
- **One** physical or click instruction at a time
- Name the control, window, or label exactly
- Block progress until confirmation (or a clear timeout + re-prompt)
- Never batch "do A, B, C, then tell me"
...
- List candidates with **plain-language likelihood** ("likely the board", "likely a different adapter")
- Prefer **explicit** IDs over blind `auto` when more than one candidate exists
- On failure, say **what you will try next** — do not shame the operator
...
|-------------|-----------------|
| Stack trace as the product UX | Short failure reason + **what to do next** |
| Warning then continue on identity mismatch | Fail closed; require explicit override only if safe and documented |
| "Error: failed" with no recovery | Numbered recovery steps the agent can drive or the human can do |

Exit codes should be stable enough for agents; messages stable enough for operators.
...
- One verb that orchestrates
- Or one command at a time with explanation, run by the agent when possible

If a tool must print a command for the human to paste (e.g. browser device code), print **that one string** with context — not five unlabeled blocks.
...
- One update/deploy/verify command (or short script)
- What "good" looks like (LED pattern, health URL, version string)
...
- Be non-interactive by default when flags can replace prompts
- Use clear exit codes (0 success, non-zero classes for retry vs human-needed vs hard fail)
- Print operator prose on stderr or a dedicated summary stream; keep machine-readable output optional (`--json`) without making JSON the only recovery path
- Refuse or loudly warn on dangerous blind defaults (e.g. auto-pick among many devices)
...
| Cloud first-deploy | Auth device code, project/account pick, region, fail-closed IAM |
| Data / ML bootstrap | Runtime/env install, GPU checks, one re-run training/serve command |
...
## Surface checklist (this layer only)
...
- [ ] Fail closed on identity/verify mismatches (or documented exception)
- [ ] Success leaves one Day-2 command and what "good" looks like

Contract pin: [`contract/`](../contract/).  
Prove manners: [`eval/`](../eval/).
